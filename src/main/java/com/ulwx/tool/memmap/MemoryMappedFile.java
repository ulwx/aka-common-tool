...
public class MemoryMappedFile {
    ...
    private void init(String shareDirPath, String sf, Integer initSize) throws Exception {
        ...
        checkDataIntegrity(writeMetaInfoList, readMetaInfoList);
        this.closed = false;
    ...
    public synchronized int getLastReadIndexEnd() {
        return this.lastReadIndexEnd;
    }

    public synchronized int getLastReadIndexStart() {
        return lastReadIndexStart;
    }

    public synchronized DataType getLastReadType() {
        return lastReadType;
    }

    public synchronized int getLastWriteIndexEnd() {
        return this.lastWriteIndexEnd;
    }

    public synchronized int getLastWriteIndexStart() {
        return lastWriteIndexStart;
    }


    public synchronized DataType getLastWriteType() {
        return lastWriteType;
    }

    private void checkDataIntegrity(List<WriteMetaInfo> writeMetaInfolist,
    ...
    private void setLastReadPosition(int startpos, int endpos, DataType dt) throws Exception {
        ...
        rmapBuf.position(this.rmapBufNextWritePos);
        rmapBuf.put(bytes);
    ...
    public static ReadMetaInfo readLastReadPosition(RandomAccessFile RRAFileMeta, String fileName,
                                                    List<ReadMetaInfo> readMetaInfoList) throws Exception {
    ...
    private void setLastWritePosition(int startpos, int endpos, DataType dt) throws Exception {
        ...
        wmapBuf.position(this.wmapBufNextWritePos);
        wmapBuf.put(bytes);
        this.wmapBufNextWritePos = (this.wmapBufNextWritePos + PACKET_HEADER_LEN + META_DATA_LEN) % META_MAP_FILE_LEN;

    }

    public static WriteMetaInfo readLastWritePosition(RandomAccessFile WRAFileMeta, String fileName,
    ...
    private void write(byte[] buff, DataType dt) throws Exception {
        ...
        if (postion > fsize - 1 || postion + len > fsize) {
            ...
            if (delta < REDUCE_LIMIT && (delta - 1) <= this.getLastReadIndexStart()) { //进行缩容
                ...
                this.mapBuf.position(this.getLastReadIndexEnd() + 1);
                this.mapBuf.get(copyBytes);
                this.mapBuf.position(0);
    ...
    public synchronized void readString(LogicFun lf) throws Exception {
    ...
    public synchronized String readString() throws Exception {
        int postion = this.getLastReadIndexEnd() + 1;
        FData fdata = this.read(postion);
    ...
    public synchronized WriteMetaInfo getWriteMetaInfo() throws Exception {
    ...
    private FData read(int postion) throws Exception {
        try {
            ...
            if (postion > this.getLastWriteIndexEnd()) {
                //确定是否文件已写满
                throw new MMException(MMException.ERROR_CODE_WRITE_FULL,
