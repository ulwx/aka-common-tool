...
public class ExcelUtilGt2007 {

	protected Logger logger = Logger.getLogger(ExcelUtilGt2007.class);

	public static InputStream getExcelStream(String sheetName, String title[], List datalist) {
		try {
			XSSFWorkbook wb = new XSSFWorkbook();
			XSSFSheet sheet = wb.createSheet(sheetName);
			XSSFRow[] row = new XSSFRow[datalist.size() + 1];// 行
			XSSFCell[] valueCell = new XSSFCell[title.length];// 列
			row[0] = sheet.createRow(0);
			// 创建列头
			XSSFCell[] headerCell = new XSSFCell[title.length];
	...
	public boolean writeExcel(String path, String[] title, List datalist) {
		...
		try {
			...
			row[0] = sheet.createRow(0);
			// 创建列头
			XSSFCell[] headerCells = new XSSFCell[title.length];
			...
			while (iterator.hasNext()) {
				...
				if(elmeRow instanceof Object[]) {
					...
					XSSFCell[] columCells = new XSSFCell[datas.length];// 列
					row[n_row] = sheet.createRow(n_row);
					...
					row[n_row] = sheet.createRow(n_row);
					XSSFCell[] columCells = new XSSFCell[list.size()];// 列
		...
		try {
			...
			for (int irow = aSheet.getFirstRowNum(); irow <= aSheet.getLastRowNum(); irow++) {
				if (null == aSheet.getRow(irow))
					continue;
				XSSFRow aRow = aSheet.getRow(irow);
				...
				for (int icel = 0; icel < maxColumNum; icel++) {
					...
					if (null != aRow.getCell(icel)) {
						XSSFCell aCell = aRow.getCell(icel);
						try {
	...
	public List<Map<String,Object>>  readExcelByTitle(String path, int numSheets) {
		...
		try {
			...
			// 读取标题行
			XSSFRow firstRow = aSheet.getRow(0);
	...
	public static void main(String[] arg) {
		...
		new ExcelUtilGt2007().writeExcel("c:\\1.xls", title, datalist);
