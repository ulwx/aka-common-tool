 ...
 * 2016年8月29日
 */
public class EasyPoiExcelBatchExportService extends ExcelExportService implements IWriter<Workbook> {
    ...
    public static  EasyPoiExcelBatchExportService build(SXSSFWorkbook workbook,String sheetName, Class<?> pojoClass){
        EasyPoiExcelBatchExportService exportService = new EasyPoiExcelBatchExportService();
        exportService.init(workbook,sheetName, pojoClass);
        return exportService;
    }
    public void init(SXSSFWorkbook workbook,String sheetName, Class<?> pojoClass) {

        ExportParams detailParams = new ExportParams();
        detailParams.setSheetName(sheetName);
        List<ExcelExportEntity> excelParams = createExcelExportEntityList(detailParams, pojoClass);
        init(workbook,detailParams, excelParams);

    }


    private void init(SXSSFWorkbook workbook,ExportParams entity, List<ExcelExportEntity> excelParams) {
        ...
        createSheet(workbook, entity);
        if (entity.getMaxNum() == 0) {
            entity.setMaxNum(USE_SXSSF_LIMIT);
        }
        insertDataToSheet(workbook, entity, excelParams, null, sheet);
    }
    private void init(XSSFWorkbook workbook,ExportParams entity, List<ExcelExportEntity> excelParams) {
        LOGGER.debug("ExcelBatchExportServer only support SXSSFWorkbook");
        entity.setType(ExcelType.XSSF);
        workbook = workbook;
        this.entity = entity;
    ...
    private List<ExcelExportEntity> createExcelExportEntityList(ExportParams entity, Class<?> pojoClass) {
        try {
    ...
    private void createSheet(Workbook workbook, ExportParams entity) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Excel export start ,List<ExcelExportEntity> is {}");
            LOGGER.debug("Excel version is {}",
                    entity.getType().equals(ExcelType.HSSF) ? "03" : "07");
        }
        if (workbook == null || entity == null ) {
